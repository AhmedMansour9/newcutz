...
import com.cairocart.Model.ProductDeal_Response
...
class getAllProducts_ViewModel: ViewModel() {
    ...
    fun getFiltertionData(page:String,Lang:String,token:String, MaxPrice:String,
                          MinPrice:String,
                          InCart:String,
                          InFavourit:String,
                          Limited:String,
                          NewProduct:String,
                          BestProduct:String,cat_id:String?,sub_id:String?,offer:String): LiveData<AllProducts_Response> {
        listProductsMutableLiveData = MutableLiveData<AllProducts_Response>()
        getFilterionProduct(page,Lang,token,MaxPrice,MinPrice,InCart,InFavourit,Limited,NewProduct,BestProduct,cat_id,sub_id,offer)
        return listProductsMutableLiveData as MutableLiveData<AllProducts_Response>
    }
...
//    }
    ...
    private fun getFilterionProduct(page:String,Lang: String,userId:String?,MaxPrice:String,
                                    MinPrice:String,
                                    InCart:String,
                                    InFavourit:String,
                                    Limited:String,
                                    NewProduct:String,
                                    BestProduct:String,cat_id:String?,sub_id:String?,offer:String) {
        var map= HashMap<String,String>()
        map.put("page",page)
        MaxPrice?.let { map.put("max", it) }
        MinPrice?.let { map.put("min", it) }
        InCart?.let { map.put("is_cart", it) }
        Limited?.let { map.put("is_limit", it) }
        InFavourit?.let { map.put("is_favoirt", it) }
        NewProduct?.let { map.put("is_new", it) }
        BestProduct?.let { map.put("best_seller", it) }
        cat_id?.let { map.put("category_id", it) }
        sub_id?.let { map.put("subcategory_id", it) }
        offer?.let { map.put("is_offer", it) }


        var service = ApiClient.getClient()?.create(Service::class.java)
        val call = service?.SarchFilterProducts(Lang,"Bearer "+userId,map)
        call?.enqueue(object : Callback, retrofit2.Callback<AllProducts_Response> {
            override fun onResponse(call: Call<AllProducts_Response>, response: Response<AllProducts_Response>) {

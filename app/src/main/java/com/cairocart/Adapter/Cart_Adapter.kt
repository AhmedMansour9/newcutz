...
class Cart_Adapter (context: Context, var user: MutableList<AllProducts_Response.Data.Data>)
    ...
    override fun onBindViewHolder(holder: Cart_Adapter.ViewHolder, position: Int) {
        holder.bindItems(user.get(position))
        holder.itemView.img_delete.setOnClickListener(){
            plusId_View.delete(user.get(position).id.toString())
            user.drop(position)
